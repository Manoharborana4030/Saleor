...
class Logout(BaseMutation):
    """Mutation that Delete token and make user logout from system"""
    class Arguments:
        email=graphene.String(required=True, description="Email of a user.")
    msg=graphene.String(description="Return msg after succefully logout")

    class Meta:
        description = "Delete JWT token."

    def perform_mutation(cls,root,email, **data):
        user=get_user_model().objects.get(email=email)
        token_obj=models.CustomAccessToken.objects.filter(user_id=user.id)
        if token_obj:
            for i in token_obj:
                i.delete()
                break
            ...
            msg="you already logout please login again"
            return Logout(msg=msg)
...
class CreateToken(BaseMutation):
    ...
    def perform_mutation(cls, root, info, **data):
        ...
        print(user,"user_graphql")
        access_token = create_access_token(user)
        custom_access_token=models.CustomAccessToken(custome_token=access_token,user=get_user_model().objects.get(email=user))
        custom_access_token.save()
